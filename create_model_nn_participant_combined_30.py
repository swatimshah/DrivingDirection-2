...
orig_epochs = loadtxt('combined_1_2_3_4.csv', delimiter=',', skiprows=1)
print(orig_epochs.shape)

# scale the data with robust scaler
my_epochs = rScaler.fit_transform(orig_epochs[0:1032, 0:4096])  # noise
...
input = train_data[:, 0:4096]
testinput = X_test_tmp[:, 0:4096]

#=====================================

# Model configuration

input = input.reshape(len(input), 64, 64)
input = input.transpose(0, 2, 1)
print (input.shape)

testinput = testinput.reshape(len(testinput), 64, 64)
...
# Create the model
model=Sequential()
model.add(Conv1D(filters=22, kernel_size=9, kernel_regularizer=L2(0.0002), bias_regularizer=L2(0.0002), activity_regularizer = L2(0.0002), kernel_constraint=min_max_norm(min_value=-0.4, max_value=0.4), padding='valid', activation='relu', strides=1, input_shape=(64, 64)))
model.add(Conv1D(filters=22, kernel_size=9, kernel_regularizer=L2(0.0002), bias_regularizer=L2(0.0002), activity_regularizer = L2(0.0002), kernel_constraint=min_max_norm(min_value=-0.4, max_value=0.4), padding='valid', activation='relu', strides=1))
model.add(AveragePooling1D(pool_size=3))
model.add(Conv1D(filters=22, kernel_size=6, kernel_regularizer=L2(0.0002), bias_regularizer=L2(0.0002), activity_regularizer = L2(0.0002), kernel_constraint=min_max_norm(min_value=-0.4, max_value=0.4), padding='valid', activation='relu', strides=1))
